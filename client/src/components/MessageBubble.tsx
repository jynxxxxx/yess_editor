...
export function AIBubbleStreaming({ text }: { text: string  | React.ReactNode }) {
  return (
    <div className="self-start max-w-[80%]">
      <div className="inline-block bg-neutral-100 px-4 py-2 rounded-2xl shadow-sm animate-fade-in">
        <span>{text}</span>
      </div>
    </div>
  );
      ...
      if (currentHighlight) {
        elements.push(
          <Tooltip key={elements.length} title={currentHighlight}>
            <span 
              className={isOrig ? "bg-red-400 text-white" : "bg-green-600 text-white"}
